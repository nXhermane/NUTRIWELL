...
export class MealCategory extends Entity<IMealCategory> {
  public validate(): void {
    ...
    if (Guard.isEmpty(this.props.code).succeeded)
      throw new EmptyStringError(
        "The code of mealCategory can't be empty. Please give a correct code to MealCategory"
      );
    if (Guard.isEmpty(this.props.name).succeeded)
      throw new EmptyStringError(
        "The name of mealCategory can't be empty. Please give a correct name to MealCategory"
      );
    this._isValid = true;
  }
  setName(name: string) {
  ...
  static create(
    props: IMealCategory,
    generateUnitqueId: GenerateUniqueId
  ): Result<MealCategory> {
    try {
      const mealId = generateUnitqueId.generate().toValue();
      const mealCategory = new MealCategory({ props, id: mealId });
      return Result.ok<MealCategory>(mealCategory);
    } catch (error) {
      return error instanceof ExceptionBase
        ? Result.fail<MealCategory>(`[${error.code}]:${error.message}`)
        : Result.fail<MealCategory>(
            `Erreur inattendue. ${MealCategory.constructor.name}`
          );
    }
  }
}
