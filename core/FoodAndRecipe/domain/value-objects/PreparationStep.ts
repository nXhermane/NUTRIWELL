...
export class PreparationStep extends ValueObject<IPreparationStep> {
   ...
   validate(props: IPreparationStep) {
      if (Guard.isEmpty(props.description).succeeded) throw new EmptyStringError("The description of preparation step can't be empty.");
      if (Guard.isNegative(props.stepNumber).succeeded)
         throw new NegativeValueError("The step number of preparation step can't be a negative number.");
   }
