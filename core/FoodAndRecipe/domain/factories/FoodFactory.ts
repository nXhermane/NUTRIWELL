...
import { FoodGroupRepository, FoodUnitValidator, NutrientRepository } from "../../infrastructure";
...
export class FoodFactory implements Factory<CreateFoodProps, Food> {
   ...
   async create(props: CreateFoodProps): Promise<Result<Food>> {
      try {
         ...
         const availbleUnits = await this.dependencies.foodUnitValidator.getAvailableUnits();
