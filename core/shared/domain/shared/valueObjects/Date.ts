  ...
  protected validate(props: Readonly<DomainPrimitive<string>>): void {
    ...
    if (!validFormat) {
      throw new InvalidArgumentFormatError(
        "Format de date invalide. Utilisez le format YYYY-MM-DD, DD/MM/YYYY ou MM/DD/YYYY."
      );
    }
    ...
    if (parts[0].length === 4) {
      ...
      throw new InvalidArgumentFormatError(
        "Format de date invalide. Assurez-vous que l'année est au format YYYY."
      );
    }
    ...
    if (isNaN(year) || isNaN(month) || isNaN(day)) {
      throw new ArgumentInvalidException(
        "Date invalide. Assurez-vous que l'année, le mois et le jour sont des nombres valides."
      );
    }

    if (month < 1 || month > 12) {
      throw new ArgumentOutOfRangeException(
        "Mois de date invalide. Utilisez des valeurs entre 1 et 12."
      );
    }
    ...
    if (day < 1 || day > daysInMonth) {
      throw new ArgumentOutOfRangeException(
        "Jour de date invalide. Assurez-vous que le jour est valide pour le mois donné."
      );
    }
  }
  ...
  static create(date?: string): Result<DomainDate> {
    try {
      ...
      return e instanceof ExceptionBase
        ? Result.fail<DomainDate>(`[${e.code}]:${e.message}`)
        : Result.fail<DomainDate>(
            `Erreur inattendue. ${DomainDate?.constructor.name}`
          );
    }
  }
}
